...
export function MissoesPersistidas({ missoes = [] }) {
  const T = useTema();
    ...
    <SectionCard titulo="Missões" sub={`${fechadas} de ${missoes.length} concluída${fechadas === 1 ? "" : "s"} — fecham sozinhas quando você bate o critério.`}>
      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
        {lista.map((mi) => {
          ...
          const materia = ROTULO_MATERIA[mi.missoes?.materia_codigo] ?? mi.missoes?.materia_codigo ?? "";
            ...
            <div key={mi.id} style={{ background: T.card, border: `1px solid ${fechada ? T.green : T.line}`, borderRadius: 10, padding: "11px 13px" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                ...
                <span style={{ marginLeft: "auto" }}>
                  <StatusBadge tom={fechada ? "ok" : "alerta"}>{fechada ? "✓ Concluída" : "Em andamento"}</StatusBadge>
                </span>
              </div>
              {fechada ? (
                ...
                <div style={{ marginTop: 8 }}>
                  ...
                  <div style={{ fontSize: 11, color: T.sub, marginTop: 4 }}>
                    {mi.questoes_acumuladas} questões{mi.acuracia != null ? ` · ${mi.acuracia}% de acerto` : " · registre acertos para medir o domínio"}
                  </div>
                </div>
              )}
            </div>
