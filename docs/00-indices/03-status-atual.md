...
| Arquivos de teste | **104** (108 `.mjs` em `tests/`; `matriz-autorizacao.mjs` e `matriz-autorizacao-achados.mjs` são apoio da matriz, sem `node:test`) | medido em 24/09 (Bloco 5 sobre a main com os Blocos 1 a 4) — `grep -l 'node:test' tests/*.mjs \| wc -l` |
