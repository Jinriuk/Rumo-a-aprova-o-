...
|---|---|---|
| Testes | **1169 / 1169 verdes** | medido em 26/09 (Etapa 3: portão de jornadas, sanitização de artefatos e guarda do job e2e-local) — `bash reset-db.sh && npm test` em Postgres 16 local dedicado (migrations + seed 2×), com `PGHOST=127.0.0.1` (a matriz de autorização recusa socket) |
| Arquivos de teste | **112** (118 `.mjs` em `tests/`; 6 são apoio, sem `node:test`: `identidades.mjs`, `calendario-cn.mjs`, os dois da matriz e os dois da guarda de embed ambíguo, `embeds-postgrest.mjs` e `embeds-fks-multiplas.mjs`) | medido em 26/09 (Etapa 3: portão de jornadas e guarda do job e2e-local) — `grep -l 'node:test' tests/*.mjs \| wc -l` |
