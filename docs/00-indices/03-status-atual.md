...
| Arquivos de teste | **107** (113 `.mjs` em `tests/`; 6 são apoio, sem `node:test`: `identidades.mjs`, `calendario-cn.mjs`, os dois da matriz e os dois da guarda de embed ambíguo, `embeds-postgrest.mjs` e `embeds-fks-multiplas.mjs`) | medido em 25/09 (guarda de embed ambíguo e aluno sem trilha) — `grep -l 'node:test' tests/*.mjs \| wc -l` |
