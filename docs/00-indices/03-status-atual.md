...
|---|---|---|
| Testes | **971 / 971 verdes** | medido em 24/09 (Bloco 4) — `bash reset-db.sh && npm test` em Postgres 16 local dedicado (migrations + seed 2×) |
| Arquivos de teste | **93** (95 `.mjs` em `tests/`) | medido em 24/09 (Bloco 4) — `grep -l 'node:test' tests/*.mjs \| wc -l` |
| Lint | **0 errors / 256 warnings** | medido — `cd app && npm run lint` |
| Build de produção | **verde** | medido — `cd app && npm run build` |
| Migrations no repo | **54** (última: `0053_proximo_ciclo_semanas_seg_dom`, ainda NÃO aplicada — aguarda aprovação do D09) | medido — `ls supabase/migrations/*.sql \| wc -l` |
