...
|---|---|---|
| Testes | **1192 / 1192 verdes** | medido em 26/09 (Etapa 5: release-gate, job da matriz e guardas de migration) — `bash reset-db.sh && npm test` em Postgres 17 (container `postgres:17`, o mesmo do CI), com `app/dist` buildado (migrations + seed 2×), com `PGHOST=127.0.0.1` (a matriz de autorização recusa socket) |
| Arquivos de teste | **114** (120 `.mjs` em `tests/`; 6 são apoio, sem `node:test`: `identidades.mjs`, `calendario-cn.mjs`, os dois da matriz e os dois da guarda de embed ambíguo, `embeds-postgrest.mjs` e `embeds-fks-multiplas.mjs`) | medido em 26/09 (Etapa 5: guardas do release-gate e de migrations) — `grep -l 'node:test' tests/*.mjs \| wc -l` |
