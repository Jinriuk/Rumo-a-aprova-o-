...
|---|---|---|
| Testes | **1157 / 1157 verdes** | medido em 25/09 (Etapa 3: trava de destino do E2E local) — `bash reset-db.sh && npm test` em Postgres 16 local dedicado (migrations + seed 2×), com `PGHOST=127.0.0.1` (a matriz de autorização recusa socket) |
| Arquivos de teste | **109** (115 `.mjs` em `tests/`; 6 são apoio, sem `node:test`: `identidades.mjs`, `calendario-cn.mjs`, os dois da matriz e os dois da guarda de embed ambíguo, `embeds-postgrest.mjs` e `embeds-fks-multiplas.mjs`) | medido em 25/09 (Etapa 3: trava de destino do E2E local) — `grep -l 'node:test' tests/*.mjs \| wc -l` |
