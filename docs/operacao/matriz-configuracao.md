...
| `PGHOST`, `PGPORT`, `PGUSER`, `PGPASSWORD`, `PGDATABASE` | conexão do Postgres de teste | `tests/reset-db.sh`, suíte `node --test` | L | obrigatório para a suíte | `ci.yml:56-60` (build-e-unitarios) e `ci.yml:244-248` (matriz-autorizacao); `.env.example` | arquivo | **ok**: `build-e-unitarios` verde no run `36221482457` | repo | 26/09 |
